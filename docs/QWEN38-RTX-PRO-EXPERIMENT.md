...
unit. The production service is recorded as Qwen3.5 GPTQ-Int4 on vLLM with a
65,536-token configured context; this experiment evaluates a corrected Qwen3.8
GGUF under llama.cpp with the large embedding tensor on CPU and two
131,072-token slots. The Q4_K_M weights alone are about 119 GB, so they cannot
fit entirely in GPU memory. CPU offload, available host RAM, actual allocation,
...
The dedicated overrides are in
[`../infra/experiments/qwen38-rtx-test.tfvars`](../infra/experiments/qwen38-rtx-test.tfvars).
They select one on-demand `1RTXPRO6000.30V` with tensor parallelism 1 for the
...
The 119 GB checkpoint needs about 140 GB free disk space. Record actual host
RAM with `free -h`; require roughly 80 GiB free before trying the CPU-resident
embedding table, then confirm actual memory use after load. The embedding
override may still leave too much of the remaining weights on the GPU. Proceed
only if logs show successful placement without CUDA out-of-memory or automatic
context reduction. If CUDA development tools are absent, install a toolkit
compatible with the installed driver before building.
